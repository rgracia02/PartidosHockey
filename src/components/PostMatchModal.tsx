      ...
      <div className="bg-[#F2F2F7] dark:bg-slate-950 w-full max-w-lg rounded-t-[32px] p-5 pb-safe max-h-[92vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-200">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-200/80 shrink-0 dark:border-slate-800">
          <div>
            ...
            <h3 className="text-lg font-black text-slate-900 tracking-tight dark:text-white">Planilla de Cierre</h3>
          </div>
          <button
            onClick={onClose}
            className="p-2 rounded-full bg-slate-200/80 text-slate-700 hover:bg-slate-300 active:scale-95 transition-all dark:text-slate-300"
          >
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-4 no-scrollbar">
          ...
          <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-200/80 dark:bg-slate-900 dark:border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-4 items-center">
              {/* Team A Counter */}
              <div className="flex flex-col items-center text-center p-2 rounded-2xl bg-slate-50 border border-slate-100 dark:bg-slate-800 dark:border-slate-800">
                <div className="flex items-center gap-1.5 mb-2 max-w-full">
                  <span
                    className="w-3 h-3 rounded-full shrink-0 shadow-xs"
                    style={{ backgroundColor: teamA?.color || '#0284c7' }}
                  />
                  <span className="font-bold text-xs text-slate-800 truncate dark:text-slate-200">{nameA}</span>
                </div>

                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setScoreA((prev) => Math.max(0, prev - 1))}
                    className="w-10 h-10 rounded-2xl bg-white border border-slate-200 shadow-xs flex items-center justify-center font-bold text-slate-700 active:bg-slate-100 active:scale-90 transition-all text-lg dark:bg-slate-900 dark:text-slate-300 dark:bg-slate-800 dark:border-slate-700 dark:active:bg-slate-700"
                  >
                  ...
                  <input
                    ...
                    className="w-12 text-center text-3xl font-black text-slate-900 bg-transparent focus:outline-none dark:text-white"
                  />
                  <button
              ...
              <div className="flex flex-col items-center text-center p-2 rounded-2xl bg-slate-50 border border-slate-100 dark:bg-slate-800 dark:border-slate-800">
                <div className="flex items-center gap-1.5 mb-2 max-w-full">
                  ...
                  />
                  <span className="font-bold text-xs text-slate-800 truncate dark:text-slate-200">{nameB}</span>
                </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                ...
                {isShootout && (
                  <div className="mt-3 p-3 bg-violet-50/80 dark:bg-violet-950/30 rounded-2xl border border-violet-100 dark:border-violet-900/60 space-y-3">
                    ...
                    <div>
                      <label className="block text-[10px] text-slate-600 font-bold mb-1 dark:text-slate-400">
          ...
          <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-200/80 dark:bg-slate-900 dark:border-slate-800">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100 mb-3 dark:border-slate-800">
              <div className="flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-slate-50/80 rounded-2xl p-2.5 border border-slate-100 dark:bg-slate-800 dark:border-slate-800">
                <p className="text-[11px] font-bold text-slate-700 mb-2 truncate flex items-center gap-1 dark:text-slate-300">
                  <span
                ...
                <div className="space-y-1 max-h-36 overflow-y-auto no-scrollbar">
                  {teamA?.players.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className="flex items-center justify-between bg-white px-2.5 py-1.5 rounded-xl border border-slate-100 text-xs shadow-2xs dark:bg-slate-900 dark:border-slate-800"
                      >
                        ...
                        <div className="flex items-center gap-1">
                          {goal && (
                            <button
                              ...
                              className="w-5 h-5 rounded bg-slate-100 text-slate-600 font-bold flex items-center justify-center text-xs dark:text-slate-400 dark:bg-slate-800"
                            >
                              -
                            </button>
                          )}
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="flex-1 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-medium text-slate-800 min-h-[44px] dark:text-slate-200 dark:bg-slate-800 dark:border-slate-700"
                >
          ...
          <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-200/60 dark:bg-slate-900 dark:border-slate-800">
            ...
            {photoUrl ? (
              <div className="relative rounded-2xl overflow-hidden border border-slate-200 aspect-video bg-black/5 flex items-center justify-center group dark:border-slate-700">
                <img
                  src={photoUrl}
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button
                    ...
                    className="p-2 bg-white rounded-full text-slate-800 text-xs font-bold flex items-center gap-1 shadow-md dark:bg-slate-900 dark:text-slate-200"
                  >
        ...
        <div className="pt-3 border-t border-slate-200/80 shrink-0 flex flex-col gap-2 dark:border-slate-800">
          ...
          <button
            ...
            className="w-full py-3 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-800 font-bold rounded-2xl text-xs flex items-center justify-center gap-2 active:scale-[0.98] transition-all min-h-[42px] dark:text-slate-200 dark:bg-slate-800"
          >
            <Check className="w-4 h-4 text-slate-600 stroke-[2.5] dark:text-slate-400" />
